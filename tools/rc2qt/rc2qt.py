...
class rcStringTable(object):
    StringTableItemFormat = """   qtMfcStringResources.insert({0},"{1}");"""
    table = None
    ...
    def output(self):
        for i in range(len(self.table)):
            print(
                self.StringTableItemFormat.format(
...
"""
    ...
    def output_decl(self):
        print(
            self.ToolbarsDeclItemFormat.format(
                self.id, self.numbers[self.X], self.numbers[self.Y]
            )
        )

    def output_impl(self):
        print(
            self.ToolbarsImplHeaderFormat.format(
                self.id, self.numbers[self.X], self.numbers[self.Y]
            )
        )
        idx = 0
        for item in self.table:
            if item.BUTTON():
                print(
                    self.ToolbarsImplItemFormat[self.BUTTON].format(
                        idx,
                        item.ID().getText(),
                        self.numbers[self.X],
                        self.numbers[self.Y],
...
{{
	// BEGIN"""
    AcceleratorTablesImplItemFormat = """   // {1}, {0}, {2}
	{{ {2}, {1}, {0} }},
"""
    AcceleratorTablesImplFooter = """   // END
	{ 0, 0, 0 },
};
"""
    AcceleratorTablesDeclItemFormat = """      case {0}:
			return ACCEL_{0};
			break;
"""
    table = None
    ...
    def output_impl(self):
        ...
        for item in self.table:
            ...
            print(
                self.AcceleratorTablesImplItemFormat.format(
                    item.ID().getText(), eventString, optionString
                )
            )
        ...
        for child in popup.children:
            if type(child) is rcParser.Menu_itemContext:
                ...
                if child.SEPARATOR():
                    ...
                    print(
                        self.MenuItemFormat[self.MENUITEM].format(
                            child.ID().getText(), child.String().getText(), optionString
                        )
                    )
...
"""
    CaptionFormat = """   // CAPTION {0}
	parent->SetWindowText({0});
"""
    StyleFormat = """   // STYLE {0}
	parent->toQWidget()->setWindowFlags(parent->toQWidget()->windowFlags(){1});
"""
    FontFormat = """   // FONT {0}, {1}, {2}, {3}, {4}
"""
    DialogExImplHeaderEnd = """   // BEGIN"""
    DialogExDeclItemFormat = """      case {0}:
			qtMfcInitDialogResource_{0}(parent);
			break;
"""
    DialogExImplFooter = """   // END
}
"""
    id = ""
    items = None
            ...
            if option.STYLE():
                ...
                if stylesOrig:
                    ...
                    styles = styles.replace(
                        "WS_MAXIMIZEBOX", "Qt::WindowMaximizeButtonHint"
                    )
                    styles = styles.replace(
                        "WS_MINIMIZEBOX", "Qt::WindowMinimizeButtonHint"
                    )
                if styles:
                    styles = styles.split("|")
    ...
    def output_generic_control_statement(self, idx, item):
        ItemFormat = """   // CONTROL {3}, {1}, {4}, {2}, {6}, {7}, {8}, {9}
...
"""
        ItemFormatNoTitle = """   // CONTROL {3}, {1}, {4}, {2}, {6}, {7}, {8}, {9}
	{5}* mfc{0} = new {5}(parent);
	CRect r{0}(CPoint({6}, {7}), CSize({8}, {9}));
	parent->MapDialogRect(&r{0});
...
"""
    ...
    def output_groupbox_control_statement(self, idx, item):
        ItemFormat = """   // GROUPBOX {3},{1},{4},{5},{6},{7}
...
"""
    ...
    def output_combobox_control_statement(self, idx, item):
        ItemFormat = """   // COMBOBOX {1},{3},{4},{5},{6},{2}
...
"""
    ...
    def output_listbox_control_statement(self, idx, item):
        ItemFormat = """   // LISTBOX {1},{3},{4},{5},{6},{2}
...
"""
    ...
    def output_defpushbutton_control_statement(self, idx, item):
        ItemFormat = """   // DEFPUSHBUTTON {3},{2},{4},{5},{6},{7}
	CButton* mfc{0} = new CButton(parent);
	CRect r{0}(CPoint({4},{5}),CSize({6},{7}));
	parent->MapDialogRect(&r{0});
	mfc{0}->Create(_T({3}),{2},r{0},parent,{1});
...
"""
        styles = self.get_styles(
            item.stylelist(), "BS_DEFPUSHBUTTON|WS_VISIBLE|WS_BORDER"
        )
...
"""
    ...
    def output_dialog_item(self, idx, item, groupboxPriority):
        if groupboxPriority == True:
            if item.groupbox_control_statement():
                _ = self.output_groupbox_control_statement(
                    idx, item.groupbox_control_statement()
                )
                return 1
            return 0
        subId = None
        if item.generic_control_statement():
            subId = self.output_generic_control_statement(
                idx, item.generic_control_statement()
            )
        elif item.static_control_statement():
            subId = self.output_static_control_statement(
                idx, item.static_control_statement()
            )
        elif item.combobox_control_statement():
            subId = self.output_combobox_control_statement(
                idx, item.combobox_control_statement()
            )
        elif item.listbox_control_statement():
            subId = self.output_listbox_control_statement(
                idx, item.listbox_control_statement()
            )
        elif item.edit_control_statement():
            subId = self.output_edit_control_statement(
                idx, item.edit_control_statement()
            )
        elif item.button_control_statement():
            subId = self.output_button_control_statement(
                idx, item.button_control_statement()
            )
        elif item.defpushbutton_control_statement():
            subId = self.output_defpushbutton_control_statement(
                idx, item.defpushbutton_control_statement()
            )
        if subId:
    ...
    def output_impl(self):
        print(
            self.DialogExImplHeaderStartFormat.format(
                self.id, self.x, self.y, self.dx, self.dy
            )
        )
        print(self.caption)
        print(self.style)
...
class myRcVisitor(rcVisitor):
    ...
    def visitAccelerator_statement(self, ctx):
        acceleratorTableList.add(
            rcAcceleratorTable(ctx.ID().getText(), ctx.accelerator_item())
        )
    ...
    def visitDialogex_statement(self, ctx):
        dialogExList.add(
            rcDialogEx(
                ctx.ID(0).getText(),  # ID(0) is dialog ID, ID(1) is help ID
                ctx.dialog_item(),
                ctx.dialog_option(),
                ctx.Number(),
            )
    ...
    def swap_order(self, d, wsz=4, gsz=2):
        return "".join(
            [
                "".join([m[i : i + gsz] for i in range(wsz - gsz, -gsz, -gsz)])
            ...
            for item in ctx.dialoginit_item():
                if item.dialoginit_data():
                    ...
                    for data in item.dialoginit_data():
                        itemToAdd += (
                            data.getText().strip(",").replace("0x", "").strip('"\\')
                        )
                        if len(itemToAdd) % 2:
                            itemToAdd += "0"
                    ...
                    dialogInitList.addItem(
                        ctx.ID().getText(), item.ID().getText(), itemToAdd
                    )
